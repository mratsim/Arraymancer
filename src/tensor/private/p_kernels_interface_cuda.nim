...
# Does C[i] = alpha `op` B[i]

template cuda_lscal_binding(kernel_name: string, binding_name: untyped)=
  # Generate a Nim proc that wraps the C++/Cuda kernel proc
...
template cuda_lscal_glue*(
  ...
  inline void """, kernel_name,"""(
    ...
    const T * __restrict__ src_data){
      ...
      cuda_apply_lscal<<<blocksPerGrid, threadsPerBlock>>>(
        rank, len,
        dst_shape, dst_strides, dst_offset, dst_data,
        alpha,
        """,op_name,"""<T>(),
