...
proc check_reshape(t: Tensor, new_shape:seq[int]) {. inline.}=
...
proc check_squeezeAxis(t: AnyTensor, axis: int) {. inline.}=
  if axis >= t.rank:
    raise newException(ValueError, "The axis is out of range, axis is " & $axis & " while the tensor rank is " & $t.rank )

proc check_unsqueezeAxis(t: AnyTensor, axis: int) {. inline.}=
...
proc transpose*(t: Tensor): Tensor {. inline.}=
  ## Transpose a Tensor.
  ##
...
proc unsafeTranspose*(t: Tensor): Tensor {. inline.}=
...
proc reshape*(t: Tensor, new_shape: varargs[int]): Tensor {. inline.}=
  ## Reshape a tensor
  ##
  ## Input:
  ##   - a tensor
...
proc unsafeBroadcast*[T](t: Tensor[T], shape: varargs[int]): Tensor[T] =
  ## Explicitly broadcast a Tensor to the specified shape.
...
proc unsafeBroadcast2[T](a, b: Tensor[T]): tuple[a, b: Tensor[T]] =
...
proc exch_dim(t: Tensor, dim1, dim2: int): Tensor =
  if dim1 == dim2:
    return
...
proc unsafeSqueeze*(t: Tensor): Tensor =
  ## Squeeze tensors. For example a Tensor of shape @[4,1,3] will become @[4,3]
  ## Input:
  ##   - a tensor
  ## Returns:
...
proc squeeze*(t: AnyTensor, axis: int): AnyTensor =
...
proc unsafeSqueeze*(t: Tensor, axis: int): Tensor =
  ## Collapse the given axis, if the dimension is not 1; it does nothing
  ## Input:
  ##   - a tensor
  ##   - an axis (dimension)
...
proc unsqueeze*(t: AnyTensor, axis: int): AnyTensor =
