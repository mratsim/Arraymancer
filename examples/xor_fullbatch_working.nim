...
# We will pass full batches of 3200 samples
let bsz = 3200

# We create it as int between [0, 2[ (2 excluded) and convert to bool
let x_train_bool = randomTensor([bsz, 2], 2).astype(bool) # generate batch_size examples of (0,1) combination
...
for epoch in 0..<5:
  ...
  let n1 = linear(x_train, layer_3neurons)
  let n1_act = n1.relu
  let n2 = linear(n1_act, classifier_layer)
  let loss = sigmoid_cross_entropy(n2, y)
